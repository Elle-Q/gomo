...
func (s *CategoryService) Save(cat *models.Category) *CategoryService {
	if err := s.CatHandler.Save(cat).Error; err != nil {
		_=s.AddError(err)
	}
	return s
}

func (s *CategoryService) Delete(id int) *CategoryService{
	if err := s.CatHandler.Delete(id).Error; err != nil {
		_=s.AddError(err)
	}
	return s
}
